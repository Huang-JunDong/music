...
import { downloadDir, getWebSettings } from "@/lib/store";
import { loadDownloadDedupSet } from "@/lib/download-record";
import { checkSaveLocalGuard, wantsSaveLocal } from "@/lib/write-guard";
import { isLocalMusicSource, localMusicTrackByID, trackAbsPath } from "@/lib/local-music";
import { newSourceRangeFetch, rangeFetchStream } from "@/lib/range-fetch";
...
async function handle(req: NextRequest): Promise<NextResponse> {
  ...
  if (source === "soda") {
    const { fetchDecryptedSodaAudio } = await import("@/lib/providers/soda");
  ...
  try {
    ...
    return new NextResponse("Failed to get URL", { status: 404 });
  }
  ...
  // 对齐 Go：WebDAV 配置且非 stream 播放时，整段缓冲后上传（失败仅 warning 头）
  let buffered: Uint8Array | null = null;
  if (webdavConfigured(settings) && !streamPlayback) {
    try {
    ...
    if (upstream.status >= 200 && upstream.status < 300) {
      try {
        await uploadSongToWebDAV(settings, filename, Buffer.from(buffered));
      } catch (err) {
        headers.set("X-MusicDL-Warning", String(err instanceof Error ? err.message : err));
      }
    }
  }

  if (buffered) {
    return new NextResponse(new Uint8Array(buffered), { status: upstream.status, headers });
  }
 ...
 * 对齐 Go downloadHandler：save_local（写盘+去重+WebDAV+JSON）/ embed=1（元数据嵌入）/
 * soda（服务端解密）/ 普通 Range 透传 / 本地文件（304+206+save_local JSON）。
 */
