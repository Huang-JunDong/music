/** 应用配置 — 对齐 api-enhanced util/config.json APP_CONF */
...
/** 评论 resourceType 映射 — 对齐 config.json resourceTypeMap（原名字面量导出） */
...
/** 别名（大写风格） */
export const RESOURCE_TYPE_MAP = resourceTypeMap;

/** 特殊路由（对齐 server.js special：文件名 → HTTP 路由，不做下划线转斜杠） */
